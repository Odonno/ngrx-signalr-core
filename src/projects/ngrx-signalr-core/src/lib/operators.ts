...
export function ofHub({
  hubName,
  url,
}: HubKeyDefinition): MonoTypeOperatorFunction<HubAction>;
export function ofHub(
  ...
  if (typeof x === "string") {
    ...
    return filter(
      (action) => action.hubName === x.hubName && action.url === x.url
    );
  }
}
...
type ObservableMapHubToActionFunc<T extends Action, U> = (
  input: ObservableMapHubToActionInput<T>
) => Observable<U>;

const hubAndActionOrNotFound = <T extends Action, U>(
  func: ObservableMapHubToActionFunc<T, U>
) => (action: T) => {
 ...
 */
export const mergeMapHubToAction = <T extends Action, U>(
  func: ObservableMapHubToActionFunc<T, U>
 ...
 * @param func A function that returns an Observable according to the given action and SignalR hub instance.
 */
export const switchMapHubToAction = <T extends Action, U>(
  func: ObservableMapHubToActionFunc<T, U>
 ...
 * @param func A function that returns an Observable according to the given action and SignalR hub instance.
 */
export const exhaustMapHubToAction = <T extends Action, U>(
  func: ObservableMapHubToActionFunc<T, U>
